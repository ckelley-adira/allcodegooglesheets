...
| OQ-08 | **Pre-K only vs. mixed grade layouts.** Resolved: Added `preKOnlyMode` feature flag. Pre-K–only sites set this flag to `true` to use Pre-K Data/Pacing/Summary sheets. Mixed-grade sites that include Pre-K leave it `false` and add "PreK" to grades. A "prek" layout option is now available in `SHEET_LAYOUT_OPTIONS`. | Resolved | copilot | Phase 7d |
| OQ-09 | **Site onboarding branding block.** Resolved: Onboarding allows defaults. `getBranding()` already returns safe defaults when the branding block is empty. SiteConfig_TEMPLATE.gs now documents this explicitly. Full branding is optional during setup. | Resolved | copilot | Phase 7d |
| OQ-10 | **Autofill logic generalization for group layouts.** Resolved: `autoBalanceStudents()` now checks `MIXED_GRADE_CONFIG.combinations` when `mixedGradeSupport` is enabled, pooling students across combined grades before distributing. Pre-K groups continue to use the same round-robin logic. | Resolved | copilot | Phase 7d |
| OQ-11 | **School menu customization (icon/label) in SetupWizard.** Resolved: Added `menuCustomization` section to `SITE_CONFIG` with `menuName` and `featureLabels` overrides. `ModuleLoader.buildFeatureMenu()` reads these values via `getFeatureMenuLabel()`. Schools can override labels/icons per feature; defaults are used when not specified. | Resolved | copilot | Phase 7d |
| OQ-12 | **Version upgrade paths for legacy config.** Resolved: Added `versionTracking` section to `SITE_CONFIG` with `previousVersions`, `upgradeDate`, and `legacyConfigFormat`. Helper functions `isLegacyUpgrade()` and `getVersionTracking()` allow code to gate backward-compatible behavior for migrated sites. | Resolved | copilot | Phase 7d |
