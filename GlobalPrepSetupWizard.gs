...
// NOTE: getGroupsFromConfiguration() in this file is a duplicate of the
// shared version in SetupWizard.gs. Use the canonical version instead.
 ...
 * UPDATED: Now includes Tutoring submenu + Mind Trust reporting
 */
function onOpen() {
  ...
  if (!configSheet || !isSystemConfigured()) {
    ...
    // Configured - show full menu with Tutoring options
    ui.createMenu('Adira Reads Progress Report')
      .addItem('🔄 Sync All Progress', 'syncAllProgress')
...
function getExistingGrades(configSheet) {
  ...
  for (let i = 0; i < GRADE_OPTIONS.length; i++) {
    const value = configSheet.getRange(5 + i, 2).getValue();
...
function getExistingGradeMixing(configSheet) {
  const allowed = configSheet.getRange(18, 2).getValue();
...
/**
 * Creates or updates the Group Configuration sheet
 * Lists each group individually with student count for easy manual updates
 * @param {Spreadsheet} ss - Active spreadsheet
 * @param {Object} data - Wizard data
 */
function createGroupConfigSheet(ss, data) {
  ...
  // Build list of all individual group names
  const allGroupNames = [];
  groups.forEach(g => {
  ...
  // Also scan for any groups in student data that aren't in the config (like tutoring groups)
  ...
  // Sort groups by grade then name
  ...
  // Formatting
  sheet.setColumnWidth(1, 250);  // Group Name - wider for full names
  sheet.setColumnWidth(2, 80);   // Grade
  sheet.setColumnWidth(3, 100);  // # of Groups
  sheet.setColumnWidth(4, 100);  // Students
...
/**
 * Gets all data from a sheet as a Map keyed by student name
 * @param {Spreadsheet} ss - Active spreadsheet
 ...
 * FIXED: Scans actual Grade Group sheets to extract real group names
 *        instead of generating generic names from Group Configuration
...
function getGroupsForForm() {
  ...
  // Pattern to match grade sheet names like "KG Groups", "G1 Groups", "G3 Groups", etc.
  const gradeSheetRegex = /^(PreK|KG|G[1-8]) Groups$/;
  
  // Scan all sheets looking for grade group sheets
  const sheets = ss.getSheets();
  
  sheets.forEach(sheet => {
    const sheetName = sheet.getName();
    
    if (gradeSheetRegex.test(sheetName)) {
      // This is a grade group sheet - extract actual group names
      const data = sheet.getDataRange().getValues();
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        // Group headers contain "Group" but not "Student Name"
        // Also check that the next row contains "Student Name" to confirm it's a valid group header
        if (cellA.includes("Group") && !cellA.includes("Student")) {
          // Verify this is a group header by checking if next row has "Student Name"
          if (i + 1 < data.length) {
  ...
  // Also check Pre-K Data sheet for PreK groups if it exists
    ...
    if (headerRow >= 0) {
      ...
      preKGroups.forEach(group => {
        if (!allGroupNames.includes(group)) {
          ...
          Logger.log("Found PreK group: " + group);
        }
      });
    }
  }
  
  // If no groups found in sheets, fall back to Group Configuration
  if (allGroupNames.length === 0) {
    Logger.log("No groups found in sheets, falling back to Group Configuration");
    return getGroupsFromConfiguration();
  }
  
  // Sort groups naturally (PreK, KG, G1, G2... and then by group number)
  allGroupNames.sort((a, b) => {
    // Extract grade and rest of name
    const gradeOrder = {'PreK': 0, 'KG': 1, 'G1': 2, 'G2': 3, 'G3': 4, 'G4': 5, 'G5': 6, 'G6': 7, 'G7': 8, 'G8': 9};
    
    const gradeA = a.split(' ')[0];
    const gradeB = b.split(' ')[0];
    
    const orderA = gradeOrder[gradeA] !== undefined ? gradeOrder[gradeA] : 99;
    const orderB = gradeOrder[gradeB] !== undefined ? gradeOrder[gradeB] : 99;
    
    if (orderA !== orderB) {
      return orderA - orderB;
    }
    
    // Same grade, sort by full name
...
/**
 * Fallback function to get groups from Group Configuration sheet
...
/**
 * Gets existing Y/N/A data for a specific lesson, allowing teachers to edit previously scored lessons.
 * @param {string} groupName - The group name (e.g., "KG Group 1")
 * @param {string} lessonName - The lesson name (e.g., "UFLI Lesson 5")
 ...
 */
/**
 * Saves lesson data and updates both the Master MAP and Group Sheets instantly.
 */
/**
 * Saves lesson data submitted by the HTML form.
 * UPDATED: Redirects 'PreK' writes to 'Pre-K Data' sheet.
 */
function saveLessonData_OLD(formObject) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
