 ...
 * School-specific extensions are in: CCASkillsExtensions.gs
 ...
 * @returns {Object} Configuration object with CCA-specific constants
 */
function getCCAConfig() {
  return {
...
function syncSmallGroupProgress() {
  ...
  try {
    ...
    updateAllStats(ss, mapData, getCCAConfig());
...
function repairSkillsTrackerFormulas() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  updateAllStats(ss, null, getCCAConfig());
