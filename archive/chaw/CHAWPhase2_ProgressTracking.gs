 ...
 * School-specific extensions are in: CHAWBrandingExtensions.gs
...
function syncSmallGroupProgress() {
  ...
  progressData.forEach(row => {
    ...
    if (groupName) {
      // Extract grade prefix from group name (e.g., "G3 Group 1" -> "G3")
...
if (lessonNum) {
 ...
 Object.keys(studentCurrentLesson).forEach(name => {
  ...
  updateAllStats(ss, mapData, getCHAWConfig());
...
function repairSkillsTrackerFormulas() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  updateAllStats(ss, null, getCHAWConfig());
